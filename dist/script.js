...
window.addEventListener('DOMContentLoaded', () => {
  ...
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_1__["default"])(); // zdes mi peredali vse classi selectorov v samom faile modals.js // 

  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.glazing_slider', '.glazing_block', '.glazing_content', 'active'); // peredaem classi, soglasno selectoram v tabs.js // 

  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.decoration_slider', '.no_click', '.decoration_content > div > div', 'after_click'); // inicializiruem ostalnie tabi, chto nizhe na stranice // 
...
const forms = () => {
  ...
  const form = document.querySelectorAll('form'),
        inputs = document.querySelectorAll('input'); // proveryyaem phone inputi na chisla, vse chto ne chisla, validaciyu ne prohodit // 

  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_0__["default"])('input[name="user_phone"]'); // 2. zadaem v object soobsheniya, kotorie mi budem ispolzovat // 
  ...
  const postData = async (url, data) => {
    // ukazivaem, chto funkciya asinhronnaya // 
    document.querySelector('.status').textContent = message.loading;
    let res = await fetch(url, {
      // s pomoshiyu wait, govorim, chtobi js dozhdalsya vipolneniya fetch // 
      method: "POST",
      body: data
    });
  ...
  form.forEach(item => {
    item.addEventListener('submit', e => {
      ...
      item.appendChild(statusMessage); //sobiraem vse dannie vvedennie v formu  s pomoshiu FormData// 

      const formData = new FormData(item); // 5. Otpravlyaem dannie na server po addresu 'assets/server.php' ,s dannimi, kotorie poluchil FormData// 
...
const modals = () => {
  ...
  bindModal('.popup_calc_profile_button', '.popup_calc_end', '.popup_calc_end_close'); // showModalByTime('.popup', 60000);
};
...
const tabs = (headerSelector, tabSelector, contentSelector, activeClass, display = 'block') => {
  ...
  function showTabContent(i = 0) {
    // ukazivaem index, ili mozhno ukazat srazu index po defaultu = 0, pervi tab // 
  ...
  header.addEventListener('click', e => {
    // 1. veshaem obrabotchik sobitiya clika na obshuu oblast, v kotoroi nahodyatsya tabi // 
    const target = e.target;

    if (target && (target.classList.contains(tabSelector.replace(/\./, "")) // 2.1 proveryaem, deistvitelno li mi kliknuli v odin iz tabov //  pri pomoshi regulyarnogo virazheniya, mi prosto ubrali tochku : .class => class, t.k v classList mozhno pomestit tolko class, a ne selector // 
    || target.parentNode.classList.contains(tabSelector.replace(/\./, "")))) {
      tab.forEach((item, i) => {
        //3. perebiraem vse tabi, i zapominaem nomer kazhdogo taba po poryadku, ukazivaya ego index //  
        if (target == item || target.parentNode == item) {
          // 4. kak tolko vipolnilos uslovie v perebore // 
